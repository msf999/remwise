...
- **Each source → a document** under `Readwise/Sources` (added newest-first), tagged the **`Readwise
  Source`** powerup — one doc per book, article, tweet, podcast, or supplemental.
...
- **Highlights as bullets.** Each highlight is a plain bullet: the text with its **Readwise color**
  applied as a Remnote highlight, a trailing **📌 pin** linking back to the highlight on Readwise, and a
  **note child bullet** when the highlight has a note. A source's summary / document note is written into
  the body once, on creation.
...
│  ├─ Atomic Habits             ← tagged the "Readwise Source" powerup
│  ├─ Some Article — Jane Doe   ← disambiguated when titles collide
│  └─ …
...
│  ├─ Tweets
│  ├─ Podcasts
│  └─ Supplementals      ← Readwise's curated popular-highlight collections
...
| **Location** | → reference into `Readwise/Locations` — the Reader location (Inbox / Later / Shortlist / Archive / Feed), or _Not in Reader_ |
...
Beneath the properties sit the source's **highlights** (each a plain bullet: the highlighted text + a
📌 link back to Readwise + an optional note child) and its summary / document note (written once, on
creation). Each source also carries hidden bookkeeping slots — Readwise Book ID, Base Title, Reader Doc
...
| **Apply highlight colors** | On | Apply each highlight's Readwise color (yellow / blue / pink / orange / green / purple) as a Remnote highlight color. Off = import highlights as plain text. |
| **Include Books / Articles / Tweets / Podcasts / Supplementals** | On (each) | Five toggles — include each Readwise category when syncing. |
| **Quick sync** | Off | On = each sync only fetches sources changed since the last successful sync (fast; new tags/edits are caught). Off = fetch the whole library every time. Keep it **Off** while first working through your library (quick sync can't resurface older items you haven't synced yet); turn it **On** once you're caught up. Use the popup's **Reset last sync** button to force a full fetch. |
...
> [!NOTE]
> **Resetting last-sync:** the **Sync Readwise** popup has a confirm-gated **Reset last sync** button
> (in its status area) that clears the stored last-sync date, so your next sync re-checks your whole
> Readwise library. Already-synced items stay _Already in sync_ — nothing is re-imported or duplicated.
