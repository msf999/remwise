...
const config = {
  ...
  entry: glob.sync('./src/widgets/**/*.tsx').reduce((obj, el) => {
    ...
    // Resolve to an absolute path: newer `glob` returns entries without a leading `./`, which webpack
    // would otherwise treat as a bare module request and fail to resolve.
    const abs = path.resolve(el);
    obj[rel] = abs;
